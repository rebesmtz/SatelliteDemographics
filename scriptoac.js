...
const CensusCategories = `1. Rural Residents
...
// new functionto load the census categories
function loadCensusCategories() {
  const categories = {};
  const lines = CensusCategories.split('\n');
  lines.forEach(line => {
...
// generate treemap data - new version with all of the relevant categories of data
...
function generateTreemapData(outputAreas) {
  ...
  const censusCategories = loadCensusCategories();
  ...
  filteredFeatures.forEach(feature => {
    ...
    const population = parseInt(feature['Ethnic.group|Total|All.usual.residents'] || '0', 10);
    ...
    if (!subgroupMap.has(subgroup)) {
      subgroupMap.set(subgroup, { count: 0, population: 0 });
    }
    
    const subgroupData = subgroupMap.get(subgroup);
    subgroupData.count += 1;
    subgroupData.population += population;
  });

  // Sort supergroups based on census categories
  const sortedSupergroups = Array.from(supergroupMap.keys()).sort((a, b) => {
    return (censusCategories[a] || '').localeCompare(censusCategories[b] || '');
  });

  sortedSupergroups.forEach(supergroup => {
    const supergroupNode = { name: supergroup, children: [] };
    const groupMap = supergroupMap.get(supergroup);
    
    // Sort groups based on census categories
  ...
  Object.entries(censusCategories).forEach(([name, code]) => {
    if (code.length === 1 && !treemapData.children.some(child => child.name === name)) {
...
function createTreemap(data) {
  ...
  function render(group, root) {
    ...
    node.filter(d => d === root ? d.parent : d.children)
      ...
      node.append("title")
      .text(d => `${name(d)}\nOAs: ${format(d.value)}\nPopulation: ${format(d.data.population || 0)}`);
...
function convertTreemapToTable(treemapData) {
  ...
  tableHtml += '<tr><th>Category</th><th>OAs</th><th>Population</th></tr>';
    ...
    if (typeof node === 'object') {
      Object.entries(node).forEach(([key, value]) => {
        if (typeof value === 'object' && value !== null) {
          if (value.value !== undefined && value.population !== undefined) {
            tableHtml += `<tr><td style="padding-left: ${paddingLeft}px;">${key}</td><td>${value.value}</td><td>${value.population}</td></tr>`;
          } else {
            tableHtml += `<tr><td style="padding-left: ${paddingLeft}px;"><strong>${key}</strong></td><td></td><td></td></tr>`;
            addRowsRecursively(value, depth + 1);
          }
        } else {
          tableHtml += `<tr><td style="padding-left: ${paddingLeft}px;">${key}</td><td>${value}</td><td></td></tr>`;
        }
      });
    } else {
      tableHtml += `<tr><td style="padding-left: ${paddingLeft}px;">Value</td><td>${node}</td><td></td></tr>`;
    }
  }
...
function formatTreemapDataReadable(data, indent = '') {
  ...
  if (data.name === 'All') {
    ...
    result += `${indent}${data.name}: ${data.value} OAs, Population: ${data.population}\n`;
  }
  return result;
}
